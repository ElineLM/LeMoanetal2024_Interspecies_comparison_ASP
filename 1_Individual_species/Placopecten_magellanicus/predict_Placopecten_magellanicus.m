...
filterChecks = ...%
    f_tL10m < 0 || f_tL10m > 1.2 || ...;
    f_tL31m < 0 || f_tL31m > 1.2 || ...;
    f_tLSL < 0  || f_tLSL > 1.2  || ...;
    f_LWd < 0   || f_LWd > 1.2     ;% ||...
...
TC_tL10m   = tempcorr(temp.tL10m, T_ref, Tpars);
TC_tL31m   = tempcorr(temp.tL31m, T_ref, Tpars);
% TC_tLSL   = tempcorr(temp.tLSL, T_ref, Tpars);
...
%% 
    ...
    f = f_tL10m; TC = TC_tL10m; tL = tL10m;
    ...
    f = f_tL31m; TC = TC_tL31m; tL = tL31m;
    [t_j, ~, t_b, l_j, ~, l_b, l_i, rho_j, rho_B] = get_tj(pars_tj, f);
    rT_j = TC * rho_j * k_M; rT_B = TC * rho_B * k_M;
    L_b  = l_b * L_m; L_j = l_j * L_m; L_i = l_i * L_m;
    tT_j = (t_j - t_b) / k_M / TC;                                   % d,   time since birth at metamorphosis
    ...
    [t_j, t_p, t_b, l_j, l_p, l_b, l_i, rho_j, rho_B] = get_tj(pars_tj, f_tLSL);
      L_b = l_b * L_m;   L_j = l_j * L_m; % cm, structural length at birth and at metamorphosis
     s_M = L_j/ L_b;                 % -, acceleration factor at f
    % time = tLSL(:,1);                % d, from data
    tT = temp.tLSL;                  % K, temperature from data
    L0 = Lw0.tLSL * del_M_Pl;           % cm, initial structural length
    f = f_tLSL;                      % -, initial scaled function response is used to compute initial condition of the individual
    ...
    [t_sort_SL, it , it_sort_SL] = unique(tLSL(:,1),'sorted'); %take the time of the data
    ...
    f   = f_LWd;
    EWd = (LWd(:,1) * del_M_Pl).^3 * (1 + w * f) * d_V; % g, expected dry weight
