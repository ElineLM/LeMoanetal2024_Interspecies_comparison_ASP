...
%   f = f_tL19SA;
  ...
  Lw_b = L_b/ del_M_larv_M;                 % cm, shell height at birth
...
%   ELw_ji = Lw_i - (Lw_i - Lw_j) * exp( - rT_B * (time_ji - aT_j));  % estimated length after metamorphosis
  ...
  tT = temp.tL19SA;                     % K, temperature from data
  L0 = Lw0.tL19SA * del_M_M;            % cm, initial structural length defined in data
  ...
  [t_sort_19SA, it , it_sort_19SA] = unique(tW19SA(:,1),'sorted');
  ...
  EWd_19SA = d_V * ((LW19SA(:,1)*del_M_M ).^3  *(1 + f_tL19SA * w )) ;             %in g, dry weight of tissues and not include reproduction buffer
