...
%     ELw = L_jm/ del_M_A;                                                    % cm, physical length (shell height)
    ...
    tT = temp.tLh_C3;                                                        % K, temperature from data
    L0 = Lw0.tLh_C3 * del_M_A;                                                % cm, initial structural length defined in data
    ...
    [t_sort_tLC3, ~ , it_sort_tLC3] = unique(tLh_C3(:,1),'sorted');           %take the time of the data by ascending order, with unique values
    S_O2c = 40;                                                             % 40% as the oxygen threshold, the organism reggulatory capacity.
    S_O2 = Osat.tLh_C3;                                                 % %, oxygen saturation in the environment, from data
