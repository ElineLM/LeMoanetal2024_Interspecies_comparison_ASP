...
% metaPar.weights.p_M = 4;
metaPar.weights.p_T = 0; % need to have at least one value, thus put one that is not estimated and equal for all species
% metaPar.weights.v = 0;
...
par.z_ref = 0.5921; free.z_ref     = 1;   units.z_ref = '-';            label.z_ref = 'zoom factor';
...
par.z_species = [(par.Li_species(1)*par.del_M_P)/(par.Li_species(5)*par.del_M_Pl) ... 
                (par.Li_species(2) * par.del_M_A)/(par.Li_species(5) * par.del_M_Pl) ...
                (par.Li_species(3) * par.del_M_N)/(par.Li_species(5) * par.del_M_Pl) ...
                (par.Li_species(4) * par.del_M_M)/(par.Li_species(5) * par.del_M_Pl) ... 
                (par.Li_species(5) * par.del_M_Pl)/(par.Li_species(5) * par.del_M_Pl)]; % here 5 is Placopecten magellanicus, the reference species, thus z_species = 1
...
par.v =  0.01621;       free.v     = 1; units.v = 'cm/d';         label.v = 'energy conductance'; 
par.kap = 0.9152;    free.kap   = 1; units.kap = '-';          label.kap = 'allocation fraction to soma'; 
par.kap_R = 0.95;       free.kap_R = 0;   units.kap_R = '-';        label.kap_R = 'reproduction efficiency'; 
par.p_M = 30.97;        free.p_M   = 1; units.p_M = 'J/d.cm^3';   label.p_M = '[p_M], vol-spec somatic maint'; 
par.p_T = 0;            free.p_T   = 0;   units.p_T = 'J/d.cm^2';   label.p_T = '{p_T}, surf-spec somatic maint'; 
par.k_J = 0.002;        free.k_J   = 0;   units.k_J = '1/d';        label.k_J = 'maturity maint rate coefficient'; 
par.E_G = 5202;         free.E_G   = 1;   units.E_G = 'J/cm^3';     label.E_G = '[E_G], spec cost for structure'; 
par.E_Hb = 0.000281;    free.E_Hb  = 1;   units.E_Hb = 'J';         label.E_Hb = 'maturity at birth'; 
par.E_Hj = 0.33;        free.E_Hj  = 1;   units.E_Hj = 'J';         label.E_Hj = 'maturity at metam'; 
par.E_Hp_ref = 966.1;   free.E_Hp_ref  = 1;   units.E_Hp_ref = 'J';         label.E_Hp_ref = 'reference maturity at puberty for body size scaling'; 
...
par.h_a = 5.32e-09;     free.h_a   = 1;   units.h_a = '1/d^2';      label.h_a = 'Weibull aging acceleration'; 
...
par.del_M_larv_A = 0.8516;  free.del_M_larv_A = 0;   units.del_M_larv_A = '-';  label.del_M_larv_A = 'shape coefficient for larvae Argopecten purpuratus'; 
par.del_M_larv_M = 0.8516;  free.del_M_larv_M = 0;   units.del_M_larv_M = '-';  label.del_M_larv_M = 'shape coefficient for larvae Mimachlamys varia'; 
par.del_M_larv_N = 0.8516;   free.del_M_larv_N = 0;   units.del_M_larv_N = '-';  label.del_M_larv_N = 'shape coefficient for larvae Nodipecten subnodosus'; 
par.del_M_larv_P = 0.8516;    free.del_M_larv_P = 0;   units.del_M_larv_P = '-';  label.del_M_larv_P = 'shape coefficient for larvae Pecten maximus'; 
par.del_M_larv_Pl = 0.8516; free.del_M_larv_Pl = 0;  units.del_M_larv_Pl = '-'; label.del_M_larv_Pl = 'shape coefficient for larvae Placopecten magellanicus'; 
par.f = 1;              free.f     = 0;     units.f = '-';            label.f =         'scaled functional response for 0-var data'; 
par.f_Paracas =  1;% 1.183;      
free.f_Paracas = 1; units.f_Paracas = '-';    label.f_Paracas = 'sc func res Paracas Bay (A. purpuratus, Aguirre-Velarde et al., 2019)'; 
par.f_tL19SA =  1;%1.194;       
free.f_tL19SA = 1;  units.f_tL19SA = '-';     label.f_tL19SA =  'sc func res Bay of Brest (M. varia, Régnier-Brisson et al. in prep)'; 
par.f_AM =      1;%1.03;         
free.f_AM  = 1;     units.f_AM = '-';         label.f_AM =      'sc func res laguna Ojo de liebre (N.subnodosus, Arellano-Martinez 2011)'; 
par.f_CL =     1;% 0.8567;      
free.f_CL  = 1;     units.f_CL = '-';         label.f_CL =      'sc func res laguna Ojo de liebre and Bahia de Los Angeles (N. subnodosus, Carreno-Leon et al., 2023)'; 
par.f_RA =     1;% 0.5538;       
free.f_RA  = 1;     units.f_RA = '-';         label.f_RA =      'sc func res Bahia de Loreto (N. subnodosus, Ramirez-Arce 2009)'; 
par.f_RC =     1;% 1.108;        
free.f_RC  = 1;     units.f_RC = '-';         label.f_RC =      'sc func res Bahia Magdalena (N. subnodosus, Racotta et al., 2003)'; 
par.f_MA =     1;% 0.7896;        
free.f_MA  = 1;     units.f_MA = '-';         label.f_MA =      'sc func res Bahia Magdalena, high density (N. subnodosus, Maldonado-Amparo et al., 2004)'; 
par.f_EV =     1;% 1.096;        
free.f_EV  = 1;     units.f_EV = '-';         label.f_EV =      'sc func res Bay of Brest (P. maximus, EVECOS)'; 
par.f_B =     1;%  0.9036;        
free.f_B = 1;     units.f_B = '-';        label.f_B =     'sc func res Bay of Brest (P. maximus, Lavaud 2014)'; 
par.f_CB =    1;%  1.108;        
free.f_CB = 1;     units.f_CB = '-';        label.f_CB =     'sc func res Baie des Chaleurs (P. magellanicus, Claereboudt et al., 1994)'; 
par.f_MD10 =   1;% 1.045;      
free.f_MD10 = 1;   units.f_MD10 = '-';      label.f_MD10 =   'sc func res Newfoundland, 10 m deep (P. magellanicus, Macdonald & Thompson 1988)'; 
par.f_MD31 = 	1;%0.9524;       
free.f_MD31 = 1;   units.f_MD31 = '-';      label.f_MD31 =   'sc func res Newfoundland, 31 m deep (P. magellanicus, Macdonald & Thompson 1988)'; 
par.f_RO =     1;% 0.9247;       
free.f_RO = 1;    units.f_RO = '-';       label.f_RO =    'sc func res Gulf of Saint-Lawrence (P. magellanicus, Roddick et al., 1994)'; 
